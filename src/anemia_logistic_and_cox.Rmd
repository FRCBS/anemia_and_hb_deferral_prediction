...
#tmp <- readRDS(survival_filename) %>% ungroup() %>% rename(event_age=age)
#tmp2 <- left_join(finngen, tmp, by="donor")
...
```{r Combine datas}
filename <- sprintf("%s/combined_data.rds", data_path)
if (recompute | !file.exists(filename)) {
  combined_data <- left_join(finngen, donations) %>%
    replace_na(list(blood_donor=FALSE)) %>%
    inner_join(dosages) %>%
    inner_join(scores)
  
  combined_data <- split_set3(combined_data, seed=42, donor_field = "donor")
  saveRDS(combined_data, filename)
} else {
  combined_data <- readRDS(filename)
}
...
old_n <- nrow(combined_data)
combined_data <- combined_data %>% drop_na({{model_variables}})
...
for (g in c("male", "pre_menopausal_female", "post_menopausal_female")) {
  cat(sprintf("\n*** Group %s ***\n\n", g))
  print(summary(combined_data %>% filter(group == {{g}}) %>% mutate(across(starts_with("snp_"), as.factor))))
}
```
...
```{r, eval=FALSE}
train <- combined_data %>% filter(label == "train")
validate <- combined_data %>% filter(label == "validate")
test <- combined_data %>% filter(label == "test")
nrow(train)
nrow(validate)
nrow(test)
```
...
```{r}
old_n <- nrow(sur3)
sur <- combined_data %>% select(-age) %>% inner_join(sur3, by="donor") %>%
  mutate(across(where(is.logical), as.factor))
cat(sprintf("Dropping %i/%i rows due to combining with combined_data\n", old_n - nrow(sur), old_n))
```
...
Results didn't change much, except for BMI and Smoking variables in group all
